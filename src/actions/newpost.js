...
module.exports = bot => {
  ...
  bot.action('newpost', async (ctx) => {
    ...
    try {
        ...
        let chatInfo = await bot.telegram.getChat(channelId)
        ...
        bot.action(['-1001360694643', '-1001464750938'], async (ctx) => {
          await ctx.deleteMessage()
          ctx.reply('Agora envie um ou mais vídeos, quando concluir clique em *done*', Extra.markdown().markup(
            Markup.keyboard(['Done','Cancelar']).resize().oneTime()
          ))
